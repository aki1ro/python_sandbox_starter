...
# from nord import cli #used to interact with nord vpn 
...
anime = open('/home/media_share/anime/animewantlist.txt', "r+")


# anime = anime.readlines()
# anime = ["Boku no Hero Academia"]
#Episode checker, to see if episode already downloaded
def episode_check(el, an):
   ani = []
   for e in el:
      with open('/home/media_share/anime/animelist.txt', "r+") as al:
         for a in al:
...
def torrent_download(magnet, an):
   makedir = (f'/home/media_share/anime/{an}')
   # print(makedir)
...
#checks the driver to make sure it's running
def checkdriver():
   try:
      driver.title
      return True
